...
from django.db.models.functions import Coalesce, Q, Values
...
# from imutils.face_utils import normalize
from sklearn.preprocessing import Normalizer
...
def cam_in():
                ...
                if 0.950 <= arr[1] <= 1:
                    if registration.objects.filter(id=id1).exists():
                        ...
                        if r == None:
                            ...
                            print(f"Welcome {id1} Your IN Attendace has been Marked For today")
                        elif id1!='1':
                            ...
                            if prev_status!=status or datetime.now().astimezone().date()!=prev_date.date():
                                ...
                                emp_img_string1=buffer1.tobytes()
                                attendance.objects.create(emp_id=reg_obj,status=status,image=emp_img_string1,date_time=dt_string1)
                                print(f"Attendance IN Marked For the employee {id1}")
                        elif id1=='1':
                            ...
                            if u == None:
                                ...
                                print("Todays first Unknown Person deteced with high confidence rate at the entrance camera")
                            else:
                                ...
                                if prev_status!=status or seconds > 300:
                                    retv, buffer1=cv2.imencode('.jpg', arr[2])
                                    emp_img_string1 = buffer1.tobytes()
                                    unknown.objects.create(status=status,image=emp_img_string1,date_time=dt_string1)
                                    print("Unknown Person deteced with high confidence rate at the entrance camera")
                    else:
    ...
    # detection_process = Process(target=face_detection_process, args=(queue,))
...
def cam_out():
                    ...
                    if registration.objects.filter(id=id2).exists():
                        ...
                        if (id2!='1' and r_out!=None):
                            ...
                            if u_out == None:
                                ...
                                if prev_status_out!=status_out or seconds_out > 300:
    ...
    cap_out = cv2.VideoCapture("rtsp://admin:admin123@10.8.21.47:554/cam/realmonitor?channel=1&subtype=1")
    ...
    while cap_out.isOpened():
        ...
        retd_out, frame2_out = cv2.imencode('.jpg', frame_out)
        yield (b'--frame1\r\n'
...
#             print("CAM NOT OPEND")
#             break
#         frame, id1 ,resconfi = detect(frame0 , face_detector , face_encoder , encoding_dict)
#         if resconfi == 'unknown':
        
#             # No face detected, yield the frame without modifications
#             retd1, frame1 = cv2.imencode('.jpg', frame0)
#             yield (b'--frame1\r\n'
#                 b'Content-Type: image/jpg\r\n\r\n' + frame1.tobytes() + b'\r\n')
#             continue
#         if 'confidence' in resconfi:
#             confidence = resconfi['confidence']
#             if 0.997 <= confidence <= 1:
...
#                             u= unknown.objects.filter().order_by('-date_time').first()
#                             if u == None:
#                                 retv, buffer2=cv2.imencode('.jpg', frame0)
#                                 emp_img_string1 = buffer2.tobytes()
#                                 unknown.objects.create(status=status,image=emp_img_string1,date_time=dt_string1)
#                             else:
#                                 prev_status=u.status
...
#                                 seconds=sec.total_seconds()
#                                 if prev_status!=status or seconds > 300:
#                                     retv, buffer1=cv2.imencode('.jpg', frame0)
#                                     emp_img_string1 = buffer1.tobytes()
#                                     unknown.objects.create(status=status,image=emp_img_string1,date_time=dt_string1)
...
#                     pass         
#         else:
#             print("confidence key not found in the dictionary")
#         retd1, frame1 = cv2.imencode('.jpg', frame)
#         yield (b'--frame1\r\n'
#             b'Content-Type: image/jpg\r\n\r\n' + frame1.tobytes() + b'\r\n')
#     cap.release()
...
# def cam_out():
#     # cap = cv2.VideoCapture("http://panasonic:mohalipanasonic@10.8.21.36/nphMotionJpeg?Resolution=640x480&Quality=Standard")
#     cap = cv2.VideoCapture(0)
#     # cap = cv2.VideoCapture("/dev/video2")
#     # cap = cv2.VideoCapture("rtsp://admin:admin123@10.8.21.48:554/cam/realmonitor?channel=1&subtype=1")
#     dat = timezone.now()
#     dt_string2 = dat.strftime("%Y-%m-%d %H:%M:%S:%f")
#     try :
#         while cap.isOpened():
#             ret,frame0 = cap.read()
#             if not ret:
...
#                 break
#             status='OUT'
#             frame, id2 ,resconfi = detect(frame0 , face_detector , face_encoder , encoding_dict)
#             if 'confidence' in resconfi:
#                 confidence = resconfi['confidence']
#                 if 0.997 <= confidence <= 1:
#                     if registration.objects.filter(id=id2).exists():
#                         reg_obj = registration.objects.get(id=id2)
#                         r = attendance.objects.filter(emp_id=reg_obj).order_by('-date_time').first()
#                         if (id2!='1' and r!=None):
#                             prev_status=r.status
#                             prev_date=r.date_time.date()
#                             current_date= timezone.now().date()
#                             if prev_status=='IN' and prev_date == current_date:
#                                 retv, buffer2=cv2.imencode('.jpg', frame)
#                                 emp_img_string2=buffer2.tobytes()
#                                 attendance.objects.create(emp_id=reg_obj,status=status,image=emp_img_string2,date_time=dt_string2)
#                         elif (id2=='1' and r==None):
#                             retv, imag1=cv2.imencode('.jpg', frame0)
#                             emp_img_string2 = imag1.tobytes()
#                             unknown.objects.create(status=status,image=emp_img_string2,date_time=dt_string2)
#                         elif id2=='1':
#                             u= unknown.objects.filter().order_by('-date_time').first()
#                             if u == None:
#                                 retv, imag=cv2.imencode('.jpg', frame)
#                                 emp_img_string2 = imag.tobytes()
#                                 unknown.objects.create(status=status,image=emp_img_string2,date_time=dt_string2)
#                             else:
#                                 prev_status=u.status
#                                 p_date=u.date_time
...
#                                 seconds=sec.total_seconds()
#                                 if prev_status!=status or seconds > 300:
#                                     retv, imag3=cv2.imencode('.jpg', frame)
#                                     emp_img_string2 = imag3.tobytes()
#                                     unknown.objects.create(status=status,image=emp_img_string2,date_time=dt_string2)
#                 else:
#                     print("You are not in")
#             else:
#                 print("confidence key not found in the dictionary")
#             ret, frame2 = cv2.imencode('.jpg', frame)
#             yield (b'--frame2\r\n'
...
#     def face_detection_process(queue):
#         # cap = cv2.VideoCapture("rtsp://admin:admin123@10.8.21.48:554/cam/realmonitor?channel=1&subtype=1")
#         # cap = cv2.VideoCapture("/dev/video2")
#         cap = cv2.VideoCapture(0)
#         while cap.isOpened():
...
#             if not queue.empty():
#                 arr=queue.get()
#                 print(type(arr), type(arr[0]), type(arr[1]), type(arr[2]))
#                 face_img = arr[0]
#                 face_normalized = normalize(face_img)
#                 face_resized = cv2.resize(face_normalized, required_shape)
#                 face_d = np.expand_dims(face_resized, axis=0)
