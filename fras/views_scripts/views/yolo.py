...
import pytz, os,cv2,pyttsx3,time,shutil,base64,json,threading,mtcnn,pickle
...
def user_register(request):
    ...
    paginator = Paginator(employees, 6)  # You can adjust the page size as needed
...
def cam_in():
        ...
        while True:
            if not queue.empty():
                ...
                if registration.objects.filter(id=id1).exists():
                    ...
                    if r == None:
                        retv, buffer1 = cv2.imencode('.jpg', arr[1])
                        ...
                        if u == None:
                            retv, buffer2=cv2.imencode('.jpg', arr[1])
    ...
    while True:
        _, img = cap.read()
        face_results = face_detector.predict(img, verbose=False)
        for face_r in face_results:
            ...
            for face_box in face_boxes:
                ...
                # retd2, frame2 = cv2.imencode('.jpg', img)
                # yield (b'--frame1\r\n'
                #         b'Content-Type: image/jpg\r\n\r\n' + frame2.tobytes() + b'\r\n')

                cv2.rectangle(img, (x1+25, y1+25), (x2+25, y2+25), (0, 255, 0), line_thickness)
...
def cam_out():
    ...
    face_detector_out = YOLO('yolov8n-face.pt')
                ...
                if registration.objects.filter(id=id2).exists():
                    ...
                    if (id2!='1' and r_out!=None):
                        ...
                        if prev_status_out=='IN' and prev_date_out == current_date_out:
                            retv_out, buffer2_out=cv2.imencode('.jpg', arr_out[1])
                        ...
                        retv_out, imag1_out=cv2.imencode('.jpg', arr_out[1])
                        ...
                        if u_out == None:
                            retv_out, imag_out=cv2.imencode('.jpg', arr_out[1])
                            ...
                            if prev_status_out!=status_out or seconds_out > 300:
                                retv_out, imag3_out=cv2.imencode('.jpg', arr_out[1])
    ...
    cap_out = cv2.VideoCapture("rtsp://admin:admin123@10.8.21.48:554/cam/realmonitor?channel=1&subtype=1")

    # cap_out = cv2.VideoCapture(0)
    # cap = cv2.VideoCapture('/dev/video0')
    while True:
        _, img_out = cap_out.read()
        face_results_out = face_detector_out.predict(img_out, verbose=False)
        for face_r_out in face_results_out:
            face_boxes_out = face_r_out.boxes
            for face_box_out in face_boxes_out:
                fb_out = face_box_out.xyxy[0]
                fc_out = face_box_out.cls
                # Check if the detected object is a face (modify class index accordingly)
                if int(fc_out) == 0:  # Assuming 'face' class index is 0
                    # Get face region coordinates in the original image
                    x1_out = int(fb_out[0])
                    y1_out = int(fb_out[1])
                    x2_out = int(fb_out[2])
                    y2_out = int(fb_out[3])
                    face_img_out = img_out[y1_out:y2_out, x1_out:x2_out]
                    ar_out=(face_img_out, img_out)
                    queue_out.put(ar_out)
                ar_out=(face_img_out, img_out)
                queue_out.put(ar_out)

                cv2.rectangle(img_out, (x1_out, y1_out), (x2_out, y2_out), (0, 255, 0), line_thickness_out)

        retd_out, frame2_out = cv2.imencode('.jpg', img_out)
        yield (b'--frame1\r\n'
...
def report(request):
    ...
    #     ).values('id', 'name', 'department', 'first_in', 'last_out', 'in_count', 'out_count', )
    #     # all_post = registration.objects.all()
    #     # paginate = Paginator(all_post,20)
    #     # page_num = request.GET.get('page')
    #     # data = paginate.get_page(page_num)
    #     context = {'data': data}
    #     return render(request, 'home/reporting.html', context)
...
def unknown_view(request):
    ...
    # Pagination
    paginator = Paginator(u, 6)  # You can adjust the page size as needed
    page_num = request.GET.get('page')
